...
function validForm(){
    // 验证端口
    var sdns_port = document.form.sdns_port.value;
    var sdnse_port = document.form.sdnse_port.value;
    if(sdns_port && !/^\d+$/.test(sdns_port) || sdns_port < 1 || sdns_port > 65535){
        alert("主服务器端口必须是1-65535的数字");
        document.form.sdns_port.focus();
        return false;
    }
    if(document.form.sdnse_enable.value == "1" && sdnse_port && (!/^\d+$/.test(sdnse_port) || sdnse_port < 1 || sdnse_port > 65535)){
        alert("第二服务器端口必须是1-65535的数字");
        document.form.sdnse_port.focus();
        return false;
    }
    if(document.form.sdnse_enable.value == "1" && sdns_port == sdnse_port){
        alert("主服务器端口和第二服务器端口不能相同");
        document.form.sdnse_port.focus();
        return false;
    }

    // 验证缓存大小
    var sdns_cache = document.form.sdns_cache.value;
    if(sdns_cache && !/^\d+$/.test(sdns_cache) || sdns_cache < 0){
        alert("缓存大小必须是非负整数");
        document.form.sdns_cache.focus();
        return false;
    }

    // 验证 TTL 值
    var ttl_fields = ['sdns_rr_ttl', 'sdns_rr_ttl_min', 'sdns_rr_ttl_max', 'sdns_rr_ttl_reply_max'];
    for(var i = 0; i < ttl_fields.length; i++){
        var field = document.form[ttl_fields[i]].value;
        if(field && !/^\d+$/.test(field) || field < 0){
            alert(ttl_fields[i] + " 必须是非负整数");
            document.form[ttl_fields[i]].focus();
            return false;
        }
    }

    // 验证最大回复 IP 数量
    var max_reply_ip = document.form.sdns_max_reply_ip_num.value;
    if(max_reply_ip && !/^\d+$/.test(max_reply_ip) || max_reply_ip < 1){
        alert("最大回复IP数量必须是正整数");
        document.form.sdns_max_reply_ip_num.focus();
        return false;
    }

    // 验证 TCP 空闲时间
    var tcp_idle_time = document.form.sdns_tcp_idle_time.value;
    if(tcp_idle_time && !/^\d+$/.test(tcp_idle_time) || tcp_idle_time < 0){
        alert("TCP空闲时间必须是非负整数");
        document.form.sdns_tcp_idle_time.focus();
        return false;
    }

    // 验证 IP 切换时间
    var ip_change_time = document.form.sdns_ip_change_time.value;
    if(ip_change_time && !/^\d+$/.test(ip_change_time) || ip_change_time < 0 || ip_change_time > 100){
        alert("IP切换时间必须是0-100毫秒");
        document.form.sdns_ip_change_time.focus();
        return false;
    }

    // 验证过期 TTL 和预取时间
    var exp_fields = ['sdns_exp_ttl', 'sdns_exp_ttl_max', 'sdns_exp_prefetch_time'];
    for(var i = 0; i < exp_fields.length; i++){
        var field = document.form[exp_fields[i]].value;
        if(field && !/^\d+$/.test(field) || field < 0){
            alert(exp_fields[i] + " 必须是非负整数");
            document.form[exp_fields[i]].focus();
            return false;
        }
    }

    // 验证广告过滤 URL
    var adblock_url = document.form.sdns_adblock_url.value;
    if(document.form.sdns_adblock.value == "1" && adblock_url && !/^https?:\/\//.test(adblock_url)){
        alert("广告过滤URL必须以http://或https://开头");
        document.form.sdns_adblock_url.focus();
        return false;
    }
...
function showMRULESList(){
    ...
    if(m_list.length == 0)
        ...
        for(var i = 0; i < m_list.length; i++){
            var adbybyrulesroad = m_list[i][0] == "0" ? "已禁用" : "已启用";
            var ipc = m_list[i][5] == "0" ? "禁用" : (m_list[i][5] == "whitelist" ? "白名单" : "黑名单");
            code +='<tr id="rowrl' + i + '">';
                                ...
                                <div class="round_bottom">
                                    ...
                                    <div class="row-fluid">
                                        ...
                                        <div class="alert alert-info" style="margin: 10px;">
                                            <input type="button" class="btn btn-success" value="SmartDNS官网" onclick="window.open('https://github.com/pymumu/smartdns')" size="0">
                                            <br /><#SmartDNS_6#>
                                        </div>
                                    </div>
                                    <div id="wnd_sm_cfg">
                                        <table width="100%" cellpadding="4" cellspacing="0" class="table">
                                            <tr>
                                                <th width="50%"><#SmartDNS_7#></th>
                                                <td id="smartdns_status" colspan="2"></td>
                                            </tr>
                                            <tr>
                                                <th width="50%"><#SmartDNS_8#></th>
                                                <td>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <span style="color:#888;"><#SmartDNS3_1#></span>
                                                    <div style="position: absolute; margin-left: -10000px;">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <span style="color:#888;"><#SmartDNS4_1#></span>
                                                    <div style="position: absolute; margin-left: -10000px;">
                                    ...
                                    <div id="wnd_sm_exp">
                                        <table width="100%" cellpadding="2" cellspacing="0" class="table">
                                            ...
                                            <tr>
                                                <th width="50%"><#SmartDNS22#></th>
                                                <td>
                                                    <input type="text" maxlength="64" class="input" size="64" name="sdns_ip_change_time" style="width: 120px" value="<% nvram_get_x('', 'sdns_ip_change_time'); %>">
                                                    <span style="color:#888;">毫秒 (0-100)</span>
                                                </td>
                                            </tr>
                                            <tr>
                                                <th width="50%"><#SmartDNS23#></th>
                                                <td>
                                                    <div class="main_itoggle">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <span style="color:#888;"><#SmartDNS27_1#></span>
                                                    <div style="position: absolute; margin-left: -10000px;">
                                    ...
                                    <div id="wnd_sm_dns">
                                        <table width="100%" cellpadding="4" cellspacing="0" class="table">
                                            <tbody>
                                                ...
                                                <tr>
                                                    <th colspan="2" style="background-color: #E3E3E3;">指定服务器组可用于单独解析gfwlist,如果不需要配合SS解析gfwlist,可以不填</th>
                                                </tr>
                                                <tr>
                                        ...
                                        <table width="100%" align="center" cellpadding="0" cellspacing="0" class="table">
                                            <tr>
                                                <td>
                                                    <center>
                                                        <input name="ManualRULESList2" type="submit" class="btn btn-primary" style="width: 100px" onclick="return markGroupRULES(this, 64, ' Add ');" value="保存上游" />
                                                    </center>
                                                </td>
                                            </tr>
                                        </table>
                                        ...
                                        <table width="100%" cellpadding="2" cellspacing="0" class="table">
                                            <tr>
                                                <td colspan="7">
                                                    ...
                                                    <a href="javascript:spoiler_toggle('script9')"><span>域名地址:</span></a>
                                            ...
                                            <tr>
                                                <td colspan="6">
                                                    <i class="icon-hand-right"></i>
                                                    <a href="javascript:spoiler_toggle('script10')"><span>IP黑名单:</span></a>
                                            ...
                                            <tr>
                                                <td colspan="6">
                                                    <i class="icon-hand-right"></i>
                                                    <a href="javascript:spoiler_toggle('script12')"><span>IP白名单:</span></a>
                                            ...
                                            <tr>
                                                <td colspan="6">
                                                    <i class="icon-hand-right"></i>
                                                    <a href="javascript:spoiler_toggle('script11')"><span>自定义设置:</span></a>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="text" class="input" size="15" name="sdns_adblock_url" style="width: 400px" value="<% nvram_get_x('', 'sdns_adblock_url'); %>" />
                                                </td>
                                            </tr>
                                            <tr>
